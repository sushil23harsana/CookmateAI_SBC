...
5. For each remaining ingredient, call search_items and pick ONE SKU: in stock, relevant, and the smallest pack that covers the needed quantity (a little leftover beats buying two packs; two small packs only if genuinely cheaper than one big). Name the pack size you chose in your summary so the user can swap variants. If nothing relevant comes back, retry ONCE with a synonym ("passata" -> "tomato puree", "scallion" -> "spring onion", "cilantro" -> "coriander"); if still nothing, substitute the closest ingredient and tell the user what you swapped and why.
